...
const STORAGE_KEY = 'bensoneria-cookie-consent';
...
export default function CookieBanner() {
  ...
  function accept() {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ choice: 'accepted', date: new Date().toISOString() }));
    setVisible(false);
  }

  function minimal() {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ choice: 'minimal', date: new Date().toISOString() }));
    setVisible(false);
  }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 animate-slide-up">
      <div
        className="w-[calc(100vw-3rem)] max-w-[300px] h-auto rounded-2xl bg-white"
      ...
      >
        <div className="flex flex-col items-center justify-between pt-9 px-6 pb-6 relative">
          ...
          <p className="w-full mb-4 text-sm text-justify text-zinc-600 leading-relaxed">
            Usamos cookies propias y de terceros (Stripe, Google Fonts) para el funcionamiento de la tienda.
            <br />
            <Link
              to="/cookies"
              className="font-semibold transition-colors hover:text-[#634647] underline underline-offset-2"
            >
              Política de cookies
            </Link>
          </p>
